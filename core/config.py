...
@dataclass
class ModelConfig:
    """Configuration for OCR model architecture."""
    ...
    @classmethod
    def load(cls, config_path: Union[str, Path]) -> "ModelConfig":
...
class TrainingConfig:
    ...
    @classmethod
    def load(cls, config_path: Union[str, Path]) -> "TrainingConfig":
        """Load configuration from JSON file."""
        config_path = Path(config_path)
        with open(config_path, "r", encoding="utf-8") as f:
            config_dict = json.load(f)
...
class DataConfig:
    ...
    @classmethod
    def load(cls, config_path: Union[str, Path]) -> "DataConfig":
        """Load configuration from JSON file."""
        config_path = Path(config_path)
        with open(config_path, "r", encoding="utf-8") as f:
            config_dict = json.load(f)
...
class VisualizationConfig:
    """Configuration for training and inference visualization."""
...
class ConfigManager:
    """Central configuration manager for the OCR system."""
    ...
    def save_all(self, base_dir: Union[str, Path]) -> None:
        ...
        with open(base_dir / "visualization_config.json", "w") as f:
            json.dump(self.visualization.to_dict(), f, indent=2)
    
    @classmethod
    def load_all(cls, base_dir: Union[str, Path]) -> "ConfigManager":
        ...
        if (base_dir / "visualization_config.json").exists():
            with open(base_dir / "visualization_config.json", "r") as f:
                viz_dict = json.load(f)
                viz_config = VisualizationConfig.from_dict(viz_dict)
