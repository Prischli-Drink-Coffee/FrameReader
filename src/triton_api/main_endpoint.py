...
class MainEndpointClient:
    ...
    async def _make_request(
        ...
        for attempt in range(self.max_retries):
            try:
                log.info(f"Making request to {endpoint} (attempt {attempt + 1}/{self.max_retries})")
                ...
                log.info(f"Request to {endpoint} completed successfully")
                return result
    ...
    async def donut_inference_from_arrays(
        self, 
        image_arrays: List[np.ndarray],
        filenames: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        log.info(f"Starting Donut inference for {len(image_arrays)} numpy arrays")
        
        files = await self._prepare_image_files_from_arrays(image_arrays, filenames)
        result = await self._make_request('/generate/donut', files)
        
        log.info(f"Donut inference completed. Processed {result.get('processed_images', 0)} arrays")
        return result
...
async def example_usage():
    """
    Примеры использования MainEndpointClient с numpy массивами
    """
    ...
    async with MainEndpointClient() as client:
        ...
        # YOLO inference с numpy массивами
        ...
        # Donut inference с numpy массивами
        ...
        # Работа с файлами (для сравнения)
