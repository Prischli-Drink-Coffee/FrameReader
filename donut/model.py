...
class DonutModel:
    ...
    def save_pretrained(self, output_dir: Union[str, Path]) -> None:
        ...
        self.model.config.decoder.max_length = self.max_length
    ...
    def token2json(self, tokens, is_inner_value=False):
        """
        Преобразует последовательность токенов в структуру JSON.
        
        Args:
            tokens (str): Токены для преобразования в JSON
            is_inner_value (bool): Флаг для рекурсивной обработки внутренних значений
            
        Returns:
            dict or list: Результирующая JSON структура
        """
        output = dict()
        ...
        if len(output):
            ...
            cleaned_tokens = re.sub(r"<[^>]*>", "", tokens).strip()
            if cleaned_tokens:
                return [{"text_sequence": cleaned_tokens}] if is_inner_value else {"text_sequence": cleaned_tokens}
            else:
