...
def parse_arguments():
    ...
    vis_group = parser.add_argument_group("Параметры визуализации")
    vis_group.add_argument("--plot_dpi", type=int, default=300,
                        help="DPI для сохранения графиков")
...
def setup_visualization(args):
    try:
        ...
        logger.warning("Не удалось настроить визуализацию: matplotlib или seaborn не установлены")
    ...
    try:
        ...
        for metric_name, values in metrics.items():
            if isinstance(values, list) and values:
                logger.info(f"{metric_name}: {values[-1]}")

        logger.info(f"Модель и графики сохранены в {output_dir}")
        ...
        if hasattr(trainer, "model"):
            try:
                ...
                logger.info(f"Прерванная модель сохранена в нулевой эпохе")
                
                if hasattr(trainer, "metrics_visualizer"):
                    trainer.metrics_visualizer.visualize_metrics(
                        trainer.global_step, 
                        start_epoch + (trainer.global_step / dataset_info["steps_per_epoch"])
                    )
                    logger.info("Сохранены текущие графики обучения")
                    trainer.metrics_visualizer.save_metrics(trainer.global_step)
